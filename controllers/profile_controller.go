...
func UpdateEmploymentHistory(w http.ResponseWriter, r *http.Request) {
	...
	if authHeader == "" {
		utils.RespondWithJSON(w, http.StatusUnauthorized, "Authorization token is required", map[string]interface{}{})
		return
	}
	...
	if tokenString == authHeader {
		utils.RespondWithJSON(w, http.StatusUnauthorized, "Invalid token format", map[string]interface{}{})
		return
	}
	...
	if len(req.EmploymentHistory) == 0 {
		utils.RespondWithJSON(w, http.StatusBadRequest, "Employment history is required", map[string]interface{}{})
		return
	}
	...
	if err != nil {
		utils.RespondWithJSON(w, http.StatusInternalServerError, "Failed to update employment history", map[string]interface{}{})
		return
	}

	// Respond with success
	utils.RespondWithJSON(w, http.StatusOK, "Employment history updated successfully", map[string]interface{}{
		"user_id":           userID,
...
func GetUserDetails(w http.ResponseWriter, r *http.Request) {
	...
	utils.RespondWithJSON(w, http.StatusOK, "User details fetched successfully", map[string]interface{}{
		"user": userDetails,
	})
}